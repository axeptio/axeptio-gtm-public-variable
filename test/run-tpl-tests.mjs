...
//
// Ported from axeptio/axeptio-gtm-public-template, which took it in turn from
// axeptio/axeptio-sgtm-public-template. Two differences from the tag repo's copy,
// both forced by this being a MACRO (a variable) rather than a TAG:
//
//   runCode() returns    the variable's value is the thing under test; a tag has
//                        no return value, so the tag runner discards it
//   getType shimmed      this template gates every cookie path on
//                        getType(x) === 'object'; the tag template never calls it
...
function buildTestApi() {
  ...
  const defaults = {
    ...
    // GTM's getType, which this template gates every cookie path on
    // (getType(x) === 'object'). Without it require('getType') falls through to a
    // bare spy returning undefined, so those paths would return undefined instead
    // of erroring — a silent pass-by-omission, the worst kind of test failure.
    // 'null' and 'array' are separate types in the sandbox, not 'object'.
    getType: (value) => {
  ...
  // Returns the template's value. This is a MACRO (a variable), so runCode()'s
  // result IS the thing under test — scenarios read it as `const variableResult =
  // runCode(mockData)`. The tag sibling this runner came from discards the
  // completion value, which is correct there and silently wrong here: every
  // scenario would assert against undefined.
  const runCode = (data) => {

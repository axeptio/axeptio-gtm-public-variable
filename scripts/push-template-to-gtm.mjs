...
//
// Ported from axeptio/axeptio-gtm-public-template's script of the same name, which
// drives the *tag* template. Copied rather than shared: both repos are public and a
// public repo cannot call reusable workflows from an internal one, so there is
// nowhere shared to put it (see docs/release-automation.md). Keep the two aligned.
//
// The one behavioural difference: this template is a MACRO (a variable), so the
// container needs no trigger for it to compile, and the script creates the template
// on first run rather than requiring a manual UI import.
//
...
const REPO = 'axeptio/axeptio-gtm-public-variable';
...
const templateName = process.env.GTM_TEMPLATE_NAME?.trim() || 'Axeptio Consent State';
...
// Creates the template when the container has never seen it. The sibling refuses and
// tells you to import it through the UI once; there is no reason to make a human do
// that, and a CI container that has been recreated should not need a manual step
// before CI works again. A freshly created template already matches template.tpl, so
// the comparison below correctly reports it as unchanged.
async function findOrCreateTemplate(workspacePath, localData) {
  const res = await api('GET', `${workspacePath}/templates`);
  const match = (res.template || []).find((t) => t.name === templateName);
  if (match) return match;

  console.log(`No template named "${templateName}" in container ${containerId}; creating it.`);
  return api('POST', `${workspacePath}/templates`, {
    body: { name: templateName, templateData: localData },
  });
}
...
async function main() {
  ...
  try {
    let template = await findOrCreateTemplate(workspacePath, localData);
